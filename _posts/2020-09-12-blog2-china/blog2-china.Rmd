...
hubei_map_data <- data.frame(hubei$name,
                 cut(hubei$confirm, c(0,10,100,500,1000,10000, 60000),
                     labels=c("1~9","10~99","100~500","500~999","1000~9999","≥10000"),
...
ggplot(hubei_map_data) +
  ...
  scale_fill_brewer(palette="Blues") +  theme(plot.title=element_text(hjust=0.5))  +
...
#rename the columns
names(lockdown_info) = c("city", "province", "start", "end", "level", "population(million)", "confirmed", "death", "recover")
...
hubei_info <- lockdown_info %>% 
  ...
  mutate(dead_rate = round(dead / confirm,3),
         population = as.numeric(gsub(",","", population))/10000,
...
```{r}
ggplot(data = (hubei_info %>% filter(city != "Wuhan") %>% head(10)), 
